...
export interface RpcInitializeBetCardProps {
  onExecute: (params: InitializeBetParams) => Promise<any>;
  disabled?: boolean;
...
export const RpcInitializeBetCard: React.FC<RpcInitializeBetCardProps> = ({
  ...
  const handleFieldChange = (field: keyof InitializeBetParams, value: any) => {
  ...
  const handleExecute = async () => {
    ...
    try {
      const { request, response } = await onExecute(createBetParams);
        ...
        {result?.response?.hash && (
          <div className="mt-4 p-4 bg-green-900/20 border border-green-700/50 rounded-lg space-y-3">
            <div className="flex items-center gap-2 text-green-400 font-medium">
              ...
              <span>Bet Initialized Successfully</span>
            </div>
